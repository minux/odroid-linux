...
inline unsigned int max98090_i2c_read(struct snd_soc_codec *codec,
		unsigned int reg)
{
	int ret;
		...
		unsigned int value)
{
	...
	if (i2c_smbus_write_byte_data(codec->control_data,
...
enum {
	JACK_NO_DEVICE		= 0,
	HEADSET_4_POLE_DEVICE	= (1 << 0),
	HEADSET_3_POLE_DEVICE	= (1 << 1),
	TVOUT_DEVICE		= (1 << 2),
	UNKNOWN_DEVICE		= (1 << 3),
};
...
static int max98090_set_playback_path(struct snd_kcontrol *kcontrol,
...
static int max98090_get_playback_path(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct max98090_priv *max98090 = snd_soc_codec_get_drvdata(codec);
...
static int max98090_set_record_path(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct max98090_priv *max98090 = snd_soc_codec_get_drvdata(codec);
...
static int max98090_get_record_path(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct max98090_priv *max98090 = snd_soc_codec_get_drvdata(codec);
...
static int max98090_add_widgets(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_add_codec_controls(codec, max98090_snd_controls,
			ARRAY_SIZE(max98090_snd_controls));

	if (ret != 0)
		printk("max98090_add_widgets: %d\n", ret);
...
static void max98090_work(struct work_struct *work)
{
	...
	printk(" %s [%d] current_jack_type_status %d\n",
			__FUNCTION__, __LINE__, current_jack_type_status);
#endif
...
static int max98090_probe(struct snd_soc_codec *codec)
{
	...
	max98090_set_playback_speaker_headset(codec);
#endif
	snd_soc_update_bits(codec, M98090_045_PWR_SYS,
			M98090_SHDNRUN, M98090_SHDNRUN);
